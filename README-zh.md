...
**质量门**是阶段之间的强制检查点——错误最好抓、漏掉代价最大的时刻。Agent 在门之间自由发挥，到了门必须停下来汇报。你看一眼，有问题就纠偏，没问题就放行。

| # | 质量门 | 拦住什么 |
|---|--------|---------|
| 1 | 需求理解 | 理解偏差、遗漏范围、错误假设 |
| 2 | 计划审批 | 路线错误、遗漏依赖、不必要的风险 |
| 3 | 方案设计 | 架构问题、没考虑备选方案 |
| 4 | 测试计划 | 覆盖不足、策略不对 |
| 5 | 运维/可观测 | 缺监控、配置问题、上线风险 |
| 6 | 代码评审 | 实现 bug、规范问题、安全隐患 |
| 7 | PR 提交 | 变更不完整、测试没过、reviewer 选错 |

不是每个任务都要过所有门。修 bug 可能只走 1、6、7；新功能才需要全走一遍。Agent 自己判断哪些门适用，不适用的标 N/A 并说明原因。
